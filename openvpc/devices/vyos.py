from vyapi.device import VyDevice, ApiResponse

import pprint

class Device_Vyos:
    def __init__(self, host, port, protocol, username=None, password=None, private_key=None, verify=False, timeout=10):
        self.device = VyDevice(hostname=host, key=password, port=port, protocol=protocol, verify=verify)
        

    @classmethod
    def instance(cls, host, port, protocol, username=None, password=None, private_key=None, verify=False, timeout=10):
    ...
    def add_sg_rule(self, vpc_name, sg_name, sg_rule_name, sg_rule_protocol, sg_rule_port, sg_rule_source_ip, sg_rule_destination_ip):
        response = self.show(['system'])
        pprint.pprint(response)
    ...
    def add_dhcp(self, vpc_name, dhcp_name, dhcp_ip_range, dhcp_gateway, dhcp_dns_servers = [], dhcp_domain_name = None, dhcp_lease_time = None, dhcp_domain_search = []):
        response = self.show(['system'])
        pprint.pprint(response)
    ...
    def add_lag_interface(self, vpc_name, lag_name, lag_interface_name, lag_interface_mode, lag_interface_speed, lag_interface_vlan = None, lag_interface_ip = None, lag_interface_description = None):
        response = self.show(['system'])
        pprint.pprint(response)
    ...
    def add_vrrp(self, vpc_name, vrrp_name, vrrp_priority, vrrp_ip, vrrp_interface, vrrp_description = None, vrrp_track = []):
        response = self.show(['system'])
        pprint.pprint(response)
    ...
    def add_nat(self, vpc_name, nat_name, nat_type, nat_interface, nat_source_ip, nat_destination_ip, nat_description = None, nat_port_forwarding = []):
        response = self.show(['system'])
        pprint.pprint(response)
